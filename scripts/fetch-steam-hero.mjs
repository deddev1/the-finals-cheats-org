...
const heroBuffer = Buffer.from(
	await fetch(STEAM_LIBRARY_HERO, {
		headers: { 'User-Agent': 'Mozilla/5.0 (compatible; TheFinalsCheatsSite/1.0)' },
	}).then((r) => {
...
for (const width of [640, 1024, 1536]) {
	...
	console.log(`✓ finals-cheats-hero-${width}w.webp (${width}x${height})`);
}

await writeFile(
	path.join(imagesDir, 'finals-cheats-hero.png'),
	await sharp(heroBuffer).png().toBuffer(),
);
await writeFile(
	path.join(imagesDir, 'finals-cheats-hero.webp'),
